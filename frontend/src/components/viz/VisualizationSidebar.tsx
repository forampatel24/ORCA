...
const CATEGORIES = [
  ...
  },
  {
    ...
    icon: "🧭",
    subs: [
      { id: "vessel", label: "Vessel", desc: "Your entered position + draggable marker" },
      { id: "ports", label: "Ports", desc: "Port markers" },
      { id: "eez", label: "EEZ", desc: "Maritime boundary (Maharashtra clip)" },
      { id: "geofence", label: "Distance / Geofencing", desc: "Live 5.4 km buffer around your vessel" },
    ],
  },
] as const

const SOURCE_LABEL: Record<string, string> = {
  pfz: "INCOIS PFZ 21 zones 06 Sep (live)",
  sst: "Copernicus NRT 0.083° SST + PFZ",
  chl: "Copernicus NRT 0.25° chl + PFZ",
  fish: "CMFRI / OBIS (pending API)",
  wind: "Open-Meteo per-cell wind field (live)",
  waves: "Copernicus WAM wave field (live)",
  currents: "Copernicus NRT uo/vo currents (live)",
  weather: "Open-Meteo 7-day wind/temp",
  sea: "Tides + CAPE + ocean",
  cyclone: "RSMC New Delhi outlook (live, empty when no system)",
  lightning: "Blitzortung strikes + CAPE risk",
  mpa: "Protected Areas (0 in Mumbai bbox - honest empty)",
  restricted: "Coastline buffer 5.4 km",
  alerts: "marine_hazards (live) + wave threshold",
  vessel: "GFW v3 fishing events (live) + your entered position",
  ports: "Port dataset (pending)",
  eez: "MarineRegions EEZ Maharashtra clip (live)",
  geofence: "PostGIS check_geofence + notify (buffer follows your vessel)",
}
