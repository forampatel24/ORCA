                ...
                if r.status_code == 200:
                    ...
                    for idx, hrs in [(idx6, 6), (idx24, 24)]:
                        out.append({
                            ...
                            "wind_speed": float(winds[idx]) if idx < len(winds) and winds[idx] is not None else (float(current.get("wind_speed_10m")) if current.get("wind_speed_10m") is not None else None),
                            "wind_direction": float(dirs[idx]) if idx < len(dirs) and dirs[idx] is not None else (float(current.get("wind_direction_10m")) if current.get("wind_direction_10m") is not None else None),
                            "temperature": float(temps[idx]) if idx < len(temps) and temps[idx] is not None else (float(current.get("temperature_2m")) if current.get("temperature_2m") is not None else None),
                            "rainfall": float(rains[idx]) if idx < len(rains) and rains[idx] is not None else (float(current.get("precipitation")) if current.get("precipitation") is not None else None),
                            "pressure": float(pressures[idx]) if idx < len(pressures) and pressures[idx] is not None else (float(current.get("surface_pressure")) if current.get("surface_pressure") is not None else None),
